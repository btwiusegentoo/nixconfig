...
To generate actual files, open this file in emacs, and execute ~M-x org-babel-tangle~ or  ~C-c C-v t~
You can install this config running  ~nixos-rebuild switch --flake~ at repository root.
Note that this requires [[Enable flakes][flakes enabled]].
Use ~SPC m .~ in doom-emacs to move faster to headlines.

If you want to take a look at my config, Please open this file in emacs, or just go [[https://btwiusegentoo.github.io/nixconfig/][here]].
I keep in sync a HTML file based on this file, thanks to [[https://github.com/fniessen/org-html-themes]]
...
** TODO Document vifm
** TODO Nigthly neovim doesn't build. Maybe just switch to available overlay
* Flake
...
Currently, I have *home-manager*, *NUR* (nix user repository), *emacs overlay*, and *nixpkgs* 20.09,unstable,master as inputs:
#+name: flake-inputs
#+begin_src nix
  inputs = {
    home-manager = {
...
#+end_src
*** neovim-unwrapped
Install latest unstable neovim 0.5 from master branch
...
Apple's emoji font is the best emoji font.(Other emoji fonts are mostly too ugly(maybe except Twitter emoji) that is totally unusable on some places like IG stories)
