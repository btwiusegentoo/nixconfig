...
Programs that doesn't have opacity option, can be set opacity using picom with following code:
#+name: picom-opacity
#+begin_src nix
...
#+end_src
Use emacs-ng with webrender
#+name: emacs-package
#+begin_src nix
programs.emacs.package = pkgs.emacsNg;
#+end_src

add this to init.el and config.el to fix nativecomp in Nix
